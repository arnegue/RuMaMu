...
mod tests_seatalk30 {
    use rumamu::seatalk::{
            ...
            seatalk_30::Sentence30,
        };

    const TEST_DATA: [u8; 3] = [0x30, 0x00, 0x08];
    ...
    fn test_seatalk_parsing() {
        ...
        let result: Result<Sentence30, ParseError> =
            Sentence30::parse_seatalk_data(test_buffer, Sentence30::LENGTH);
        match result {
            Ok(sentence30) => {
                assert_eq!(sentence30.intensity, EXPECTED_RESULT)
            }
    ...
    fn test_seatalk_creation() {
        ...
        let seatalk_sentence = Sentence30 {
    ...
    fn test_message_length() {
        ...
        let wrong_length: usize = Sentence30::LENGTH + 1;
        let test_buffer: [u8; MAX_SEATALK_LENGTH] = [Sentence30::ID; MAX_SEATALK_LENGTH];
        let result: Result<Sentence30, ParseError> =
            Sentence30::parse_seatalk_data(test_buffer, wrong_length);
        match result {
    ...
    fn test_message_id() {
        ...
        let wrong_id: u8 = Sentence30::ID + 1;
        let test_buffer: [u8; MAX_SEATALK_LENGTH] = [wrong_id; MAX_SEATALK_LENGTH];
        let result: Result<Sentence30, ParseError> = Sentence30::parse_seatalk_data(test_buffer, 5);
        match result {
